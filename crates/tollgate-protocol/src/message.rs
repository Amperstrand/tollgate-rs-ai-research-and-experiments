...
impl BalanceUpdate {
    pub fn new(ch_id: [u8; 32], balance: u64, sig: [u8; 64], net: u64) -> Self {
        Self {
            ...
            channel_id: ByteArray::from(ch_id),
            cumulative_balance: balance,
            balance_signature: ByteArray::from(sig),
            net_amount: net,
        }
    }
    pub fn encode(&self) -> Vec<u8> {
...
impl RolloverInit {
    pub fn new(old: [u8; 32], funding: Vec<u8>) -> Self {
        Self {
            ...
            old_channel_id: ByteArray::from(old),
            new_channel_funding: ByteVec::from(funding),
        }
    }
    pub fn encode(&self) -> Vec<u8> {
...
impl RolloverReady {
    pub fn new(old: [u8; 32], new_id: [u8; 32]) -> Self {
        Self {
            ...
            old_channel_id: ByteArray::from(old),
            new_channel_id: ByteArray::from(new_id),
        }
    }
    pub fn encode(&self) -> Vec<u8> {
...
impl ChannelClose {
    ...
    pub fn new(ch_id: [u8; 32], balance: u64, sig: [u8; 64], reason: u8) -> Self {
        Self {
            ...
            final_signature: ByteArray::from(sig),
            reason,
        }
    }
    pub fn encode(&self) -> Vec<u8> {
