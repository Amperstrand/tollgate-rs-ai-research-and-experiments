...
impl WifiConnector {
    ...
    pub async fn connect(
        ...
        // Clean up any existing STA for this SSID first
        self.cleanup_stale_stas_for_ssid(ssid).await?;

        // Build and execute UCI ops for STA setup
        ...
        // Check for failures (ignore wifi reload result — it may fail if no
        // STA interface comes up yet, which is normal)
        for (i, result) in results.iter().enumerate() {
            if let Err(e) = result {
                // Shell commands (wifi reload, ifup) may fail but are non-fatal
                let op = &ops[i];
                if !matches!(op, UciOp::Shell { .. }) {
                    tracing::warn!("UCI op {i} failed: {e}");
                    // Don't return error for individual ops — wifi reload may still work
                }
            }
        }

        // Commit and reload wifi
    ...
    pub async fn disconnect(&self) -> Result<(), WifiConnectError> {
        ...
        // Find and remove all STA sections
    ...
    pub async fn switch_upstream(
        ...
        // Build the switch ops: remove old STA + add new STA
    ...
    // ─── UCI op builders (public for testing) ──────────────────────────
    ...
    pub fn build_disconnect_ops(&self) -> Vec<UciOp> {
        // In production, we'd parse `uci show wireless` to find STA sections.
        // For now, this returns ops to delete known STA section patterns.
        UciOpBuilder::new()
    ...
    /// Clean up STAs for a specific SSID.
    async fn cleanup_stale_stas_for_ssid(&self, ssid: &str) -> Result<(), WifiConnectError> {
        // Use shell to find and remove STA sections matching the SSID
    ...
    /// Find the STA interface for a given radio.
    ///
    /// Uses `iw dev` to find the wireless interface associated with the radio.
    async fn find_sta_interface(&self, radio: &str) -> Result<String, WifiConnectError> {
        // Try to find via `iw dev` — look for managed/station mode interfaces
        let output = self
            .executor
        ...
        if output.success() {
            // Parse output: look for "Interface wlanX" followed by "type managed"
        ...
        // Fallback: guess based on radio name
        // radio0 → wlan0, radio1 → wlan1, etc.
    ...
    /// Wait for a STA interface to get an IP address via DHCP.
    ///
    /// Go v1: `waitForSTAIP()` — polls with cross-radio nudge + 15s fallback.
    /// Rust: Uses `ifconfig` or `ip addr` polling with timeout.
    ...
    /// Verify that the connected SSID matches the expected one.
    async fn verify_connection(
        &self,
        iface: &str,
        expected_ssid: &str,
    ) -> Result<(), WifiConnectError> {
        // Use iwgetid to check connected SSID
        let output = self
            .executor
    ...
    ///
    /// In production, this would parse `uci show wireless` to find
    /// duplicate STA sections and remove them. For now, it uses a
    /// shell one-liner similar to Go v1.
    #[must_use]
    pub fn build_cleanup_ops(&self) -> Vec<UciOp> {
        // Find all STA sections, group by SSID, keep last one
        UciOpBuilder::new()
...
mod tests {
    ...
    fn test_build_sta_uci_ops() {
        ...
        // Should have: comment + add STA section
        assert!(ops.len() >= 2);

        // First op should be a comment
        assert!(matches!(&ops[0], UciOp::Comment { text } if text.contains("TollGate-5G")));

        // Second op should be an Add with correct values
        if let UciOp::Add {
            config,
            type_name,
            name,
    ...
    fn test_build_wwan_ops() {
        ...
        // Should have: comment + add network.wwan + add_list firewall + commit network + commit firewall
        assert!(ops.len() >= 4);

        // Check the Add op for network.wwan
        ...
        // Check the AddList for firewall
    ...
    fn test_build_switch_ops() {
        ...
        // Should have an Add for the new STA
        let add_op = ops.iter().find(|op| {
    ...
    fn test_build_disconnect_ops() {
        ...
        // Should have: comment + shell command + commit
        assert!(ops.len() >= 2);
    ...
    fn test_render_sta_ops_shell_safe() {
        ...
        for cmd in &cmds {
            ...
            // Every uci set command should have its values wrapped in single quotes
            // by sh_quote(), making shell metacharacters inert
            if cmd.contains("uci set") || cmd.contains("uci add") {
                // The SSID "Test; rm -rf /" must appear inside single quotes
                let ssid_safe = cmd.contains("'Test; rm -rf /'");
                // The password "pass'word" must have the inner quote escaped
                // by sh_quote() which produces 'pass'\''word'
