...
### Phase 0 - Current State (Hand-Rolled Crypto)
...
### Phase 1 - cdk-wasm Bridge

- [ ] Host `cdk_wasm.js` + `cdk_wasm_bg.wasm` alongside demo (CashuTube pattern)
- [ ] Implement `sig_all_message_hash` (full serialized swap request hash) - closes gap G1
...
- [ ] Exact-amount mint quotes - closes G7
- [ ] CI oracle: Playwright test that calls both cdk-wasm and `crypto.js` on identical inputs, asserts byte-for-byte match
- [ ] JS test vector validation from `test-vectors.json` (deterministic fields)
