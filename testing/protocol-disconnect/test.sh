#!/usr/bin/env bash
# Integration test: Python client sends Announce + Disconnect to gateway.
#
# Asserts:
...
echo "Running client (sending Disconnect)..."
...
# 2. Gateway saw the peer announce (the Announce was received before Disconnect).
echo "$GATEWAY_LOG" | grep -q "peer announced" \
    || fail "gateway did not log peer announced"

echo "PASS: disconnect handled cleanly (no crash, peer was announced)"
